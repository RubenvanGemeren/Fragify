...
func main() {
	//StartProgram("comp_inf_12_7_2024_15_20")
	//StartProgram("comp_anc_15_4_2024_17_38")
...
func updateChoices(msg tea.Msg, m model) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "m":
			m.Chosen = false
			fmt.Println("m was pressed")
			return m, frame()
		}

	// Use this to time out the program
	case tickMsg:
		//if m.Ticks == 0 {
		//	m.Quitting = true
		//	return m, tea.Quit
...
func choicesView(m model) string {
	...
	tpl += subtleStyle.Render("j/k, up/down: select") + dotStyle +
		...
		subtleStyle.Render("m: back to menu") + dotStyle +
		subtleStyle.Render("q, esc: quit")

	var choicesList []string
	for i, choice := range getChoices() {
		choicesList = append(choicesList, checkbox(choice, m.Choice == i))
	}
	choices := fmt.Sprint(strings.Join(choicesList, "\n"))
...
func chosenView(m model) string {
	...
	// Run scoreboard file
	msg = StartProgram(getChoices()[m.Choice])
...
func getChoices() []string {
	...
	for _, file := range files {
		...
		if file.Name()[len(file.Name())-4:] == ".dem" {
			availableDemos = append(availableDemos, file.Name()[:len(file.Name())-4])
		}
	}
